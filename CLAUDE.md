...
- **The palette is rebuilt from one accent** by `menu.apply_theme()`, which every view reads through `THEME`. The accent comes from the `accent` config setting (default indigo `#5A4FCF`), is applied before the editor opens, and previewing one in the menu is undone if the user quits without saving.
- **Never hand-pick the text colour on a highlight.** `readable_on()` decides black or white from the background's relative luminance. Hard-coding it is what produced an unreadable selection on `bright_cyan`, where a legacy console overrode the foreground.
- **Backspace backs out** of menus and sliders (`BACK_KEYS`) but still deletes inside a text field, which is the one place it cannot be a navigation key.
